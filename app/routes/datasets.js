...
export default class DatasetsRoute extends Route {
  ...
  async model() {
    const datasetTypes = await this.store.query('concept', {
      sort: 'label',
      'filter[concept-schemes][:uri:]': 'http://themis.vlaanderen.be/id/concept-scheme/a76297cd-ba80-468c-bc18-4b9e1117eee9'
    });
    ...
    return datasets.filter((d) => d).sortBy('modified').reverse();
  }
}
